...
freq_abc = zeros(floor(signal_length/2), 3);
mag_abc = zeros(floor(signal_length/2), 3);
ang_abc = zeros(floor(signal_length/2), 3);
...
for idx = 1:3
	[freq_abc(:, idx), mag_abc(:, idx), ang_abc(:, idx), ~] = ...
	    f_fft_simple(input_sig(:, idx), fs, signal_length);
