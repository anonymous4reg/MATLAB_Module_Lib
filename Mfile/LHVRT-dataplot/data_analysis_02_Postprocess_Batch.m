...
% regular expression
clear;clc
RootDir = "C:\Users\ym\Desktop\vrt\";
...
for each_folder=1:length(SubFolderCell)
    ...
    clearvars -except RootDir Field1 Field2 DipCell PostfixCell SubFolderCell each_folder ...
                            ...
                            ExportEps ExportSvg ExportResolution ExportFigureFormat
	if length(sub_folder_files) == 1
	% If only ONE suitable .mat file founded, then do it.
		
		disp(append('Loading ', 'sub_folder_files', ' ...'))
...
%                         legend({'Ia', 'Ib', 'Ic'})
%                     case 13
%                         plot(t, PWM_grid_AUAD_BUBD_CUCD(:, 1)+8)
%                         grid on
%                         hold on
%                         plot(t, PWM_grid_AUAD_BUBD_CUCD(:, 2)+6)
%                         plot(t, PWM_grid_AUAD_BUBD_CUCD(:, 3)+4)
%                         plot(t, PWM_grid_AUAD_BUBD_CUCD(:, 4)+2)
%                         plot(t, PWM_grid_AUAD_BUBD_CUCD(:, 5))
%                         plot(t, PWM_grid_AUAD_BUBD_CUCD(:, 6)-2)
%                         ylabel('PWM grid side conv 6 pulses')
%                         title('\fontname{Times new roman}PWM grid side conv 6 pulses')
%                         set(gca, 'xlim', [0, t_range], 'ylim', [-2.5, 10], 'fontname', 'times new roman', 'fontsize', setFontSize)
%                         legend({'AU', 'AD', 'BU', 'BD', 'CU', 'CD'})
%                     case 14
%                         plot(t, PWM_rotor_AUAD_BUBD_CUCD(:, 1)+8)
%                         grid on
%                         hold on
%                         plot(t, PWM_rotor_AUAD_BUBD_CUCD(:, 2)+6)
%                         plot(t, PWM_rotor_AUAD_BUBD_CUCD(:, 3)+4)
%                         plot(t, PWM_rotor_AUAD_BUBD_CUCD(:, 4)+2)
%                         plot(t, PWM_rotor_AUAD_BUBD_CUCD(:, 5))
%                         plot(t, PWM_rotor_AUAD_BUBD_CUCD(:, 6)-2)
