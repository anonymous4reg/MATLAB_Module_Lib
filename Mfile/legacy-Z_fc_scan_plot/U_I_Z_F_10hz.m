...
function U_I_Z_F_10hz(NN,Data,Freq_begin,Freq_end, arg_save_dir)
...
TT=scan_0(1,((ff-f_start)/10*(NN)+1):((ff-f_start)/10*(NN)+N) );

UA=scan_0(3,((ff-f_start)/10*(NN)+1):((ff-f_start)/10*(NN)+N) );
UB=scan_0(4,((ff-f_start)/10*(NN)+1):((ff-f_start)/10*(NN)+N) );
UC=scan_0(5,((ff-f_start)/10*(NN)+1):((ff-f_start)/10*(NN)+N) );

IA=-scan_0(6,((ff-f_start)/10*(NN)+1):((ff-f_start)/10*(NN)+N) );
IB=-scan_0(7,((ff-f_start)/10*(NN)+1):((ff-f_start)/10*(NN)+N) );
IC=-scan_0(8,((ff-f_start)/10*(NN)+1):((ff-f_start)/10*(NN)+N) );
...
% save ZN;
ZPD = ZP;
% ZP_100_1690=ZP(1:160,3);
% save ZP;

% save DATA_ZPD ZPD
save(strcat(arg_save_dir, '\', 'DATA_ZPD'), 'ZPD')

ZND = ZN;
% save DATA_ZND ZND
save(strcat(arg_save_dir, '\', 'DATA_ZND'), 'ZND')
...
xlabel('频率（Hz)');
ylabel('相角（度)');
% saveas(gca, 'ZukangScanBodePN.fig')
% saveas(gca, 'ZukangScanBode.emf')
% saveas(gca, 'ZukangScanBode.png')
f_savefig(arg_save_dir, 'Positive-Sequence(250-2500Hz)', {'fig'}, 300)
...
xlabel('频率（Hz)');
ylabel('相角（度)');
% saveas(gca, 'ZukangScanBodeZN.fig')
% saveas(gca, 'ZukangScanBode.emf')
f_savefig(arg_save_dir, 'Negative-Sequence(250-2500Hz)', {'fig'}, 300)
